defmodule Rumbl.AuthTest do
  ...
  test "login puts the user into session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Rumbl.User{id: "5787ad9970be50faebca8663"})
      |> send_resp(:ok, "")

    next_conn = get(login_conn, "/")
    assert get_session(next_conn, :user_id) == "5787ad9970be50faebca8663"
  end

  test "logout drops the session", %{conn: conn} do
    logout_conn =
      conn
      |> put_session(:user_id, "5787ad9970be50faebca8663")
      |> Auth.logout()
      |> send_resp(:ok, "")
  ...
  test "call places user from session to assigns", %{conn: conn} do
    user = insert_user(%{username: "michael"})
    conn =
      conn
